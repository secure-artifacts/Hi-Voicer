...
1. 从 [GitHub Releases](https://github.com/cg202601/Hi-Voicer/releases) 下载最新的 `Hi-Voicer_0.1.0_x64-setup.exe`，也可以下载同版本 MSI 包。
...
```powershell
gh attestation verify .\Hi-Voicer_0.1.0_x64-setup.exe --repo cg202601/Hi-Voicer
```

## 当前模型策略

第一版只维护一套 Sherpa-ONNX 运行时，避免普通用户同时维护太多引擎。模型文件统一从 Hugging Face 下载；Sherpa-ONNX 运行时来自官方 GitHub Release，它是本地推理程序，不是模型源。
...
当前 0.1.0 初版使用 Sherpa-ONNX CPU 运行时，优先保证普通 Windows 电脑可安装、可运行。GPU/CUDA/DirectML 加速尚未接入，后续会在模型和运行时组合稳定后再作为独立优化项处理。
