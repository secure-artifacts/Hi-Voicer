...
1. 从 [GitHub Releases](https://github.com/cg202601/Hi-Voicer/releases) 下载最新的 `Hi-Voicer_0.2.0_x64-setup.exe`，也可以下载同版本 MSI 包。
...
```powershell
gh attestation verify .\Hi-Voicer_0.2.0_x64-setup.exe --repo cg202601/Hi-Voicer
```

## 当前模型策略

0.2.0 正式版继续维护 Sherpa-ONNX CPU 运行时，避免普通用户同时维护太多引擎。模型文件统一从 Hugging Face 下载；Sherpa-ONNX 运行时来自官方 GitHub Release，它是本地推理程序，不是模型源。
...
GPU 加速还在后续实验阶段，0.2.0 正式版默认发布 CPU 稳定路线。当前 SenseVoice + Sherpa-ONNX CPU 方案在中文转录速度和准确度之间更稳，后续会另开 GPU 后端验证路线。
