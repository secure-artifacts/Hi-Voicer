...
GPU 加速还在后续实验阶段，0.2.1 正式版默认发布 CPU 稳定路线。转录过程中不会自动下载 CUDA runtime；如果选择 CUDA 但本地 CUDA runtime 没准备好，会直接回退 CPU。当前 SenseVoice + Sherpa-ONNX CPU 方案在中文转录速度和准确度之间更稳，后续会另开 GPU 后端验证路线。
